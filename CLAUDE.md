...
- Schema/migration changes go through the flow in [docs/plans/2026-06-migrations-adoption.md](docs/plans/2026-06-migrations-adoption.md) (`pnpm db:new` → review SQL → `pnpm db:apply`).
- A new migration's filename must sort above the newest migration on `main`, or the merge-time `supabase db push` silently skips it — `pnpm db:new` stamps this correctly, and the `migration-order` CI job enforces it.
