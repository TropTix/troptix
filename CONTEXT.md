...
**Payout rail**:
The channel a payout's money actually moves on, recorded per paid request. Two rails: **manual** — a transfer by hand from the ops bank (Mercury) — and **Stripe** — a transfer into the Organization's Connect account, which Stripe then deposits to their bank. An Organization's current rail is **derived, never declared**: Stripe when it has a Connect account that can take transfers and a verified payout destination, otherwise manual. Jamaican organizers stay on the manual rail until a Stripe product can reach Jamaican banks. The request lifecycle and ledger are rail-agnostic; a Platform Owner can still pay any single request another way. _Avoid_: a stored rail preference on the Organization, "Stripe rail" for anything Global Payouts might do later.
