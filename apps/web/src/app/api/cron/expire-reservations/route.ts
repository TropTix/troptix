 ...
 * payment can never land after release. Also drains the transactional email
 * outbox — the plan's "cron → expire() + drain" — so it is the backstop that
 * delivers confirmations for the free path and any webhook-missed paid ones (the
 * webhook nudges its own inline drain for the happy path). Scheduled via Supabase
 * cron (pg_cron + pg_net) — see docs/runbooks/expire-reservations-cron.md.
 *
