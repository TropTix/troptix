 ...
 * idempotency.
 *
 * Email is server-triggered via the transactional outbox: `confirmPaid`
 * enqueues the confirmation/refund-notice atomically with fulfillment, and we
 * drain it here via `after()` (runs post-response, so it never blocks the 200 or
 * risks a dangling promise) for instant delivery on the happy path. The cron
 * backstop covers anything this misses.
 */
...
async function handleEvent(event: Stripe.Event): Promise<void> {
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      // Fulfillment enqueued the email (confirmation or refund notice) in-txn.
      // Deliver it after the response so the 200 isn't blocked; the drain is
      // idempotent, so overlapping with the cron backstop is safe.
      after(() =>
