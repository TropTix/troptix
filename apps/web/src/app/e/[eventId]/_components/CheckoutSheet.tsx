...
export default function CheckoutSheet({
  ...
  useEffect(() => {
    ...
    if (state.kind === 'order') {
      ...
      setStep('success');
      // Email is server-triggered via the transactional outbox (enqueued in the
      // fulfillment txn, drained by the webhook/cron) — the client is out of the
      // email loop entirely.
  ...
  async function handleContact(contact: ReservationContact) {
    ...
    try {
      ...
      if (isFree) {
        ...
        setStep('success');
        // Confirmation email is enqueued server-side by completeFree (outbox)
        // and delivered by the cron drain — no client trigger.
        return;
      }
