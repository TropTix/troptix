// Mock @troptix/db so the drainer doesn't require a live database. Only the
// OutboxStatus enum and the outbox-type constants (re-exported via the barrel
// mock below) are touched at module load.
...
// The real senders talk to Resend/Prisma — never load them in a unit test.
jest.mock('./email', () => ({
