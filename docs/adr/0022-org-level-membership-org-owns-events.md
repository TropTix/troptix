# 22. Org-level Membership; the Organization owns every event
...
Members act as themselves: the actor is the member's own user id, checked against their Membership at the service seam. Nobody impersonates the Owner. View-as (ADR 0018) stays a separate, read-only, platform-only concept.
