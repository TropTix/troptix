...
- **Phase B — Cut over checkout (fixes 1.1 + 1.2).** The live-flow change. **Headline contract change:** the Order is materialized only on webhook `confirm`, so `/initiate` returns a `reservationId` and post-payment pages key off it (see _Client_, above). **B1 is shipped; B2–B4 are deferred and fold into the upcoming checkout redesign** (see Status) — the new pages are built against the reservation API rather than retrofitting the old ones, and B2's server wiring lands atomically with that new client. **Execution plan: [Stage 3 — Checkout Cutover](2026-06-stage-3-checkout-cutover.md)** (realizes B2–B4, C, D).
