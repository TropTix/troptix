...
| Which configuration            | `configuration.recipient`: "Utilize this configuration if the Account will not be the Merchant of Record, like with Separate Charges & Transfers." Its capability `stripe_balance.stripe_transfers` "Enables this Account to receive /v1/transfers into their Stripe Balance." Requesting it alone fails with `capability_not_available_without_other_capability` unless Stripe has approved the platform for transfers-only accounts, so `card_payments` is requested too and never used; TropTix stays merchant of record. | [Create an account (v2), configuration.recipient](https://docs.stripe.com/api/v2/core/accounts/create?query=configuration.recipient)                                                                                                   |
...
2. **Charge model**: **separate charges and transfers**. Checkout keeps
   ...
   it.)
3. **Account shape**: **Accounts v2, recipient plus merchant configuration,
   Express dashboard.** Created with `dashboard: 'express'`, `fees_collector`
   and `losses_collector` both `'application'`, `identity.country: 'us'`,
   `configuration.recipient.capabilities.stripe_balance.stripe_transfers.requested: true`,
   and `configuration.merchant.capabilities.card_payments.requested: true`.
   The merchant capability is Stripe's precondition, not a product choice:
   the sandbox smoke on 2026-09-22 returned
   `capability_not_available_without_other_capability` for a transfers-only
   request, and the v1 API answers the same request with "Your platform needs
   approval". Nothing ever charges through the account. Prefilled with
   `display_name` (the Organization's display name), `contact_email` (the
   owner's email), `defaults.profile.business_url` (the Organization's public
   page, so onboarding does not ask for a website), and
   `metadata.organizationId`. The Express dashboard is what gives the
   organizer a place to see deposits and change their bank without TropTix
   building either.
