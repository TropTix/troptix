...
import { openCheckout, tierCard, addTickets } from '../lib/checkout';
...
test('tier edge states render correctly', async ({ page, factory }) => {
  ...
  // Gated tier stays hidden until its code is entered.
...
test('selection clamps at the per-user max and totals update', async ({
  ...
  await addTickets(page, event.tiers.ga.id, 2);
