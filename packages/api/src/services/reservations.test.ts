...
afterAll(async () => {
  ...
  // event, so drop them first by the order/reservation ids they reference (while
...
describe('completeFree', () => {
  it('materializes a free order + tickets, idempotently', async () => {
    ...
    // Materialization enqueues exactly one confirmation outbox row, atomically
    // with the order (covering the free path — no webhook fires for it).
