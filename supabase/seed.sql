...
insert into public."TicketTypes" (
  ...
  price, "priceCents", "ticketingFees", "discountCode", "eventId", "sortOrder"
) values
  -- seed_event_1: happy-path paid tiers, on sale now, plenty available
  ('seed_tt_ga',  'PAID', now(), now(), 'General Admission', 'Standard entry',       10, 500, 0, 500, 0, 0, now(), '2026-08-15 18:00:00', now(), '2026-08-15 18:00:00', 25.00, 2500, 'PASS_TICKET_FEES',   null, 'seed_event_1', 0),
  ('seed_tt_vip', 'PAID', now(), now(), 'VIP',               'VIP entry with perks',  4,  50, 0,  50, 0, 0, now(), '2026-08-15 18:00:00', now(), '2026-08-15 18:00:00', 75.00, 7500, 'PASS_TICKET_FEES',   null, 'seed_event_1', 1),

  -- seed_event_2: free RSVP tier, on sale now, organizer absorbs fees
  ('seed_tt_rsvp', 'FREE', now(), now(), 'Free RSVP', 'Reserve a free spot',          6, 300, 0, 300, 0, 0, now(), '2026-09-05 12:00:00', now(), '2026-09-05 12:00:00', 0.00, 0, 'ABSORB_TICKET_FEES', null, 'seed_event_2', 0),

  -- seed_event_3: edge-case tiers
  --   near-capacity: capacity - reserved - sold = 2  → "Only 2 left"
  ('seed_tt_near',   'PAID', now(), now(), 'Almost Gone',   'Near-capacity tier',      10, 100, 98, 100, 0, 98, now(), '2026-09-20 19:00:00', now(), '2026-09-20 19:00:00', 30.00, 3000, 'PASS_TICKET_FEES', null, 'seed_event_3', 0),
  --   sold-out: capacity == sold → availability 0
  ('seed_tt_sold',   'PAID', now(), now(), 'Sold Out',      'Fully sold tier',          4,  50, 50,  50, 0, 50, now(), '2026-09-20 19:00:00', now(), '2026-09-20 19:00:00', 40.00, 4000, 'PASS_TICKET_FEES', null, 'seed_event_3', 1),
  --   upcoming: sale window opens in the future → not yet on sale
  ('seed_tt_soon',   'PAID', now(), now(), 'Early Bird',    'Sale opens next week',    10, 200,  0, 200, 0,  0, now() + interval '7 days', '2026-09-20 19:00:00', now() + interval '7 days', '2026-09-20 19:00:00', 20.00, 2000, 'PASS_TICKET_FEES', null, 'seed_event_3', 2),
  --   gated: non-empty discountCode → hidden until 'UNLOCK2026' is entered
  ('seed_tt_gated',  'PAID', now(), now(), 'Members Only',  'Unlock with UNLOCK2026',   4,  80,  0,  80, 0,  0, now(), '2026-09-20 19:00:00', now(), '2026-09-20 19:00:00', 60.00, 6000, 'PASS_TICKET_FEES', 'UNLOCK2026', 'seed_event_3', 3);
